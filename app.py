...
def home():
    ...
    return jsonify({
        ...
        'status': 'running',
        'version': APP_VERSION,
        'description': 'ACEest Fitness v3.0.1 - Full workout logging (sets/reps/load) + adherence timeline chart',
        'features': [
            '3 fitness programs (FL / MG / BG)',
            'Extended client profile with goals',
            'Full workout session logging: type, duration, sets, reps, weight used',
            'Per-client adherence timeline chart (PNG)',
            'Weekly progress trend per client',
            'CSV export'
        ],
        ...
        'endpoints': {
            ...
            'POST /workout': 'Log workout (with sets/reps/load)',
            'GET /workouts/<client_name>': 'List workouts',
            'GET /chart/<client_name>': 'Adherence chart',
            'GET /progress-chart/<client_name>': 'Adherence timeline (PNG)',
...
def add_client():
    try:
        ...
        cur.execute("""
            ...
            (name, age, height, weight, program, calories, adherence, target_weight, target_adherence, notes, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (name, age, height, weight, program, estimated_calories, adherence,
              target_weight, target_adherence, notes, created_at))
        conn.commit()
...
def get_clients():
    ...
    rows = cur.fetchall()
    conn.close()
    clients = [{'id': r['id'], 'name': r['name'], 'age': r['age'],
                 'weight': r['weight'], 'program': r['program'],
                 'adherence': r['adherence'] or 0, 'notes': r['notes'] or '',
                 'estimated_calories': r['calories'], 'created_at': r['created_at']} for r in rows]
    ...
    try:
        ...
        return jsonify({'success': True, 'message': 'Workout logged successfully',
                        'workout': {'id': workout_id, 'client_name': client_name,
                                    ...
                                    'reps': reps, 'weight_used': weight_used, 'notes': notes}})
    except (ValueError, TypeError) as e:
        return jsonify({'success': False, 'error': f'Invalid data: {str(e)}'}), 400
...
def progress_chart_client(client_name):
    """v3.0.1 new feature: dedicated adherence timeline PNG per client"""
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("SELECT week, adherence FROM progress WHERE client_name=? ORDER BY id", (client_name,))
    rows = cur.fetchall()
    # Also get workout count per week for a secondary axis
    cur.execute("""
        SELECT date, COUNT(*) as cnt FROM workouts
        WHERE client_name=? GROUP BY date ORDER BY date
    """, (client_name,))
    workout_rows = cur.fetchall()
    conn.close()

    if not rows:
        return jsonify({'success': False, 'error': f'No data for client: {client_name}'}), 404
    ...
    try:
        ...
        color_adh = '#d4af37'
        ax1.set_xlabel('Week')
        ax1.set_ylabel('Adherence (%)', color=color_adh)
        ax1.plot(range(len(weeks)), adherence, marker='o', linewidth=2.5,
                 color=color_adh, label='Adherence')
        ax1.fill_between(range(len(weeks)), adherence, alpha=0.2, color=color_adh)
        ax1.set_xticks(range(len(weeks)))
        ax1.set_xticklabels(weeks, rotation=45, ha='right')
        ax1.set_ylim(0, 110)
        ax1.tick_params(axis='y', labelcolor=color_adh)
        ax1.set_title(f'Progress Timeline – {client_name}')
        ...
        return jsonify({'success': True, 'client_name': client_name,
                        'chart_data': f'data:image/png;base64,{img_b64}',
                        'adherence_points': len(rows),
                        'workout_sessions': len(workout_rows)})
    except Exception as e:
        return jsonify({'success': False, 'error': f'Chart failed: {str(e)}'}), 500
