...
def init_db():
    ...
    cur = conn.cursor()
    # Clients: extended with height, target_weight, target_adherence
    cur.execute("""
...
def home():
    ...
    return jsonify({
        ...
        'status': 'running',
        'version': APP_VERSION,
        'description': 'ACEest Fitness v2.2.4 - Extended client schema (height, targets) + workout session logging',
        'features': [
            '3 fitness programs (FL / MG / BG)',
            'Extended client profile: height, target weight, target adherence',
            'SQLite: clients + progress + workouts tables',
            'Workout session logging (type, duration)',
            'Per-client adherence chart',
            'CSV export'
        ],
        ...
        'endpoints': {
            ...
            'POST /add-client': 'Add client (with height & targets)',
            ...
            'GET /workouts/<client_name>': 'List workouts for client',
            'GET /chart/<client_name>': 'Per-client adherence chart',
...
def get_clients():
    ...
    clients = [{'id': r['id'], 'name': r['name'], 'age': r['age'],
                 'height': r['height'] if 'height' in r.keys() else 0,
...
def log_workout():
    """v2.2.4 new feature: log a workout session"""
    try:
        data = request.get_json() if request.is_json else request.form
        client_name = data.get('client_name', '').strip()
